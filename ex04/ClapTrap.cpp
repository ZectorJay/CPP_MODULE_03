...
/*   Updated: 2021/06/08 16:28:38 by hmickey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap( std::string name ) : _name(name) {
	...
	std::cout<<BOLDGREEN<<"ClapTrap has been built";
	std::cout<<RESET<<RED<<" ("<<name<<") "<<RESET<<std::endl;
}
...
ClapTrap::ClapTrap( ClapTrap const & src){
	std::cout<<BOLDRED<<_class_name<<" has been cloned from another one";
...
void		ClapTrap::beRepaired( unsigned int amount ){
	if (_hp + amount > _max_hp)
		amount = _max_hp - _hp;
	std::cout<<_class_name<<" ("<<GREEN<<_name<<RESET<<") repaired for ";
...
void		ClapTrap::takeDamage( unsigned int amount ){
	...
	if (damage_taken > _hp)
		damage_taken = _hp;
	if (damage_taken != _hp){
		std::cout<<_class_name<<" ("<<GREEN<<_name<<RESET<<") take ";
		std::cout<<RED<<damage_taken<<RESET<<" amount of damage (";
	}
	else
		std::cout<<_class_name<<" ("<<GREEN<<_name<<BOLDWHITE<<") destroyed ("<<RESET;
	_hp -= damage_taken;
	std::cout<<*this<<")"<<std::endl;
}